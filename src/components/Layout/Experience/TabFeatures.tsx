        ...
        <div className="recent-feat-container" ref={recentRef}>
          ...
          {proj.map((name,i) => {
            if (name.name !== 'Dictionary') return (
              <div  style={{ backgroundImage:"url("+name.image+")" }} className={`recent-feat feat-${name.name.replace(/\s+/g,'-').toLowerCase()}`}
               key={i}
               >
                <h1>{name.name}</h1>
              </div>
            ...
            {activeIndex!=0 &&( <div className="left-slide slide-btn" onClick={()=>{
             ...
             {activeIndex!=2 &&(<div className="right-slide slide-btn" onClick={()=>{
              const index = activeIndex;
              if(index < 2) setActiveIndex(index+1)
            }}></div>)}
        </div>
      </div>
    </>
